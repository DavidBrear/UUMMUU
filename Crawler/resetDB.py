...
import AppConfig;
from uummuuObjects import getDomain;
'''
...
if __name__ == '__main__':
    ...
    if raw_input().lower()[0] == 'y':
        ...
        try:
            ...
            cursor.execute("INSERT INTO sites_sitequeue(url, crawled, domain, last_crawl, date_submitted, status) VALUES('"+site+ "', 0, '"+domain+"', now(), now(), 200);");
            #domains is the list of domains to crawl
            cursor.execute("TRUNCATE sites_domains;");
            #put back this site's domain
            cursor.execute("INSERT INTO sites_domains(url, pages_crawled) values('"+domain+"', 0);");
            #worddoc is the one-to-one word to document index
            cursor.execute("TRUNCATE sites_worddoc;");
            #new_urls is the list of URLS discovered by this crawler
            cursor.execute("TRUNCATE sites_new_urls;");
